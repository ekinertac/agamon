...
// DiffRenderer converts raw unified-diff text into an NSAttributedString:
//   + lines → green, - lines → red, @@ hunks → blue, file headers → dim.
// Related: DiffListView.swift (triggers AppState.openDiff which routes here),
//          EditorPanelView.swift (detects agamon-diff:// URL scheme and shows this view),
//          AppState.openDiff (constructs the virtual URL and pushes it to openFiles).
...
struct DiffEditorView: View {
    let fileURL:  URL     // original file path
    let rootPath: String  // project root — git is invoked here
    ...
    private func load() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            // Create NSAttributedString on main thread — it isn't Sendable.
...
struct DiffRenderer {
    ...
    // Colors are hardcoded (not theme-palette) because diff semantics are universal:
    // green always means addition, red always means deletion regardless of terminal theme.
    ...
    private static func style(for line: String) -> (NSColor, NSFont) {
        ...
        if line.hasPrefix("diff ")  ||
           ...
           line.hasPrefix("---")    ||
           line.hasPrefix("+++")                         { return (dimColor,  boldFont) }
