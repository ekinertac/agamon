...
// Runs `git diff HEAD --numstat` so staged and unstaged changes are both included.
// Tapping a row calls AppState.openDiff which opens DiffEditorView in the editor panel.
// Related: FilePanelView.swift (hosts this), GitStatusView.swift (Files tab sibling),
//          DiffEditorView.swift (renders the selected file's unified diff),
//          AppState.openDiff (creates the agamon-diff:// URL that routes to DiffEditorView).
    ...
    private static func parse(_ output: String, rootPath: String) -> [DiffFileItem] {
        ...
        return output.components(separatedBy: .newlines).compactMap { line in
            ...
            guard parts.count >= 3,
                  ...
                  let deleted = Int(parts[1]) else { return nil as DiffFileItem? }
            // Renames appear as "src => dst"; take the destination.
